...
##############################################################################################################################
######### Added by Mandar Kulkarni

    def get_eta_theta_torsion_idx(self, residues=None):
...
# the six torsions and the third index is the atom involved
        ...
        for j,i in enumerate(idx_residues):
            ...
            try:
                res_plus = self.ok_residues[i+1]
                idxs_tmp  = [res.atom("P").index,res.atom("C4'").index,res_plus.atom("P").index,res_plus.atom("C4'").index]
                if(res_plus.chain.index == res.chain.index):
                   idxs[j,1] = idxs_tmp
                else:
                    pass
            ...
            try:
                ...
                #print res,res_minus, res_minus.chain.index,res.chain.index
                idxs_tmp  = [res_minus.atom("C1'").index,res.atom("P").index,res.atom("C1'").index,res_plus.atom("P").index]
            ...
            try:
                res_plus = self.ok_residues[i+1]
                idxs_tmp  = [res.atom("P").index,res.atom("C1'").index,res_plus.atom("P").index,res_plus.atom("C1'").index]
                if(res_plus.chain.index == res.chain.index):
                   idxs[j,3] = idxs_tmp
                else:
                    pass
