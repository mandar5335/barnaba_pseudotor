...
############################################################
def eta_theta_angles(filename,topology=None,residues=None,angles=None):

    """
    Calculate pseudotorsion ([eta, theta, eta', and theta']) torsion angles.
    ...
    warn = "# Loading %s \n" % filename
    sys.stderr.write(warn)
    return eta_theta_angles_traj(traj,residues=residues,angles=angles)

def eta_theta_angles_traj(traj,residues=None,angles=None):

    top = traj.topology
    # initialize nucleic class
    nn = nucleic.Nucleic(top)
    ## Changed here MK
    all_idx,rr =  nn.get_eta_theta_torsion_idx(residues)
    ...
    if(angles==None):
        ...
        for i in range(len(angles)):
            ## changed here MK
            if(angles[i] in definitions.pseudo_angles):
## changed here MK
                idx_angles.append(definitions.pseudo_angles.index(angles[i]))
            else:
                msg = "# Fatal error. requested angle \"%s\" not available.\n" % angles[i]
